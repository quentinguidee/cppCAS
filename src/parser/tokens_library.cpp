...
std::map<TokenType, std::function<Expression *(ParseTree::Node &)>> tokensLibrary{
    ...
    {TokenType::integer,    [](ParseTree::Node &node) -> Expression * { return new Integer          (std::atoi(node.getToken().getValue().c_str())); }},
    {TokenType::real,       [](ParseTree::Node &node) -> Expression * { return new Real             (std::atof(node.getToken().getValue().c_str())); }},
